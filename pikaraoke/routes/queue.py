...
class EnqueueQuery(Schema):
    ...
    autotune = fields.String(
        load_default="",
        metadata={"description": "Optional JSON AutotuneParams for this song only"},
    )
...
class EnqueueForm(Schema):
    ...
    autotune_enabled = fields.String(load_default="")
    autotune_key = fields.String(load_default="")
    autotune_scale = fields.String(load_default="")
    autotune_correction_speed = fields.String(load_default="")
    autotune_wet_dry_mix = fields.String(load_default="")
...
def _autotune_from_request(data: dict) -> dict | None:
    """Build per-song autotune from JSON blob or individual form fields."""
    raw = data.get("autotune") or ""
    if isinstance(raw, str) and raw.strip():
        return at.parse_queue_autotune(raw)
    enabled = str(data.get("autotune_enabled", "")).lower() in ("1", "true", "on", "yes")
    if not enabled:
        return None
    payload = {
        "enabled": True,
        "key": data.get("autotune_key") or "C",
        "scale": data.get("autotune_scale") or "major",
        "correction_speed": float(data.get("autotune_correction_speed") or 0.35),
        "wet_dry_mix": float(data.get("autotune_wet_dry_mix") or 1.0),
    }
    return at.parse_queue_autotune(payload)


def _do_enqueue(song: str, user: str, autotune: dict | None = None) -> str:
    k = get_karaoke_instance()
    try:
        rc = k.queue_manager.enqueue(song, user, autotune=autotune)
    except (TypeError, ValueError) as exc:
        return json.dumps({"song": song, "success": [False, str(exc)]})
    broadcast_event("queue_update")
    song_title = k.song_manager.display_name_from_path(song)
...
def enqueue(query):
    ...
    try:
        autotune = _autotune_from_request(query)
    except (TypeError, ValueError) as exc:
        return json.dumps({"song": query["song"], "success": [False, str(exc)]})
    return _do_enqueue(query["song"], query["user"], autotune)
...
def enqueue_form(form):
    ...
    try:
        autotune = _autotune_from_request(form)
    except (TypeError, ValueError) as exc:
        return json.dumps({"song": form["song_to_add"], "success": [False, str(exc)]})
    return _do_enqueue(form["song_to_add"], form["song_added_by"], autotune)
