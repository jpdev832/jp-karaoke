...
class DownloadBody(Schema):
    ...
    autotune = fields.Raw(
        load_default=None,
        allow_none=True,
        metadata={"description": "Optional per-song Auto-Tune params when queueing after download"},
    )
...
def download(form):
    ...
    autotune = None
    if queue and form.get("autotune"):
        try:
            autotune = at.parse_queue_autotune(form.get("autotune"))
        except (TypeError, ValueError):
            autotune = None

    # Queue the download (processed serially by the download worker)
    k.download_manager.queue_download(song, queue, user, title, autotune=autotune)
