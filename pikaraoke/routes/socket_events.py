...
def setup_socket_events(socketio):
    ...
    # --- JP Karaoke Auto-Tune live controls ---
    from pikaraoke.lib import autotune_control as at

    @socketio.on("autotune_set")
    def on_autotune_set(data) -> None:
        updates = data.get("params", data) if isinstance(data, dict) else {}
        try:
            at.update_live(updates)
        except (TypeError, ValueError) as exc:
            socketio.emit("autotune_error", {"error": str(exc)}, room=request.sid)

    @socketio.on("autotune_get")
    def on_autotune_get() -> None:
        socketio.emit("autotune_update", at.get_state().to_dict(), room=request.sid)
